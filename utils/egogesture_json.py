...
def convert_csv_to_dict(csv_path, subset, labels):
    ...
    for i in range(data.shape[0]):
        ...
        # DIY start
        my_class = [12, 16, 17, 18, 19, 38, 40, 41, 53, 81]
        # my_class = [12, 16, 17, 18, 19, 38, 40, 41, 53, 81, 84]
        if row[1] in my_class:
...
# DIY
def my_load_labels(label_csv_path):
    data = pd.read_csv(label_csv_path, delimiter=' ', header=None)
    labels = []
    for i in range(data.shape[0]):
        # DIY start
...
def convert_egogesture_csv_to_activitynet_json(label_csv_path, train_csv_path, 
    ...
    my_labels = my_load_labels(label_csv_path)

    dst_data = {}
    dst_data['labels'] = my_labels
    dst_data['database'] = {}
